...
Codex.ai routine (`Quackback — close ticket on PR merge`, webhook-triggered on GitHub `pull_request`
...
- Follow this repo's [AGENTS.md](../../../AGENTS.md): branch off `main`, respect the monorepo conventions
...
- Bump the version + changelog (`CHANGELOG.md` and, if user-facing, the Quackback changelog) only if this
  ticket is significant enough per the versioning rule in AGENTS.md — most bug fixes aren't.

## 4. Open the PR

- Commit and push the branch. Never add "Co-Authored-By Codex" in then commit description.
