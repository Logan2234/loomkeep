...
```json
{
  ...
  "href": "https://status.loomkeep.app"
}
```
...
| `key`                 | yes      | Which message: `maintenance_scheduled` (needs `data.start` and `data.end`, ISO datetimes, shown in each reader's time zone), `degraded_service`, or `custom` (see below).       |
...
With `"key": "custom"`, `data` holds the text itself, one entry per
language — `{ "fr": "…", "en": "…" }`. A reader gets their own language, else
English, else the first language given.

Apart from `custom`, the text never comes from Unleash, so the banner stays in
each reader's language. A malformed payload shows nothing; the browser console
