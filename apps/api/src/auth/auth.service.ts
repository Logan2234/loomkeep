...
export class AuthService {
  ...
  async register(
    ...
    try {
      ...
      // create() — the second hits the unique constraint here instead. Only
      // fold that into the expected AuthEmailAlreadyExists path when the
      // email itself collided; `username` (generated by
  ...
  async revokeOtherSessions(
    ...
    const where = { userId, id: { not: currentSessionId } };
    // Fetched before deleting so the ids can be evicted from the session
    // cache below (see SessionCacheService) — deleteMany() alone doesn't
  ...
  async resetPassword(
    ...
    // Fetched before the transaction below deletes them, so their ids can be
    // evicted from the session cache (see SessionCacheService) once revoked.
    const sessions = await this.prisma.refreshToken.findMany({
